...
gold = 100
items = [gold, "rusty sword", "wooden shield"]
...
def dice_game():
    ...
    if gamble_choice == '3':
        print_pause(items[0])
        print_pause("Would you like to play?\n")
        dice_game()
    else:
        Not_a_choice()
        dice_game()
...
def Tavern_options():
    ...
    tavern_choice = input("1. Speak to the lady\n"
    ...
    if tavern_choice == '1':
        ...
        print_pause("She tells you that people in town say the creature\n"
                    "has been coming from the left path leading out of the village")
...
def Market_options():
    ...
    market_choice = input("1. Buy the sword.\n"
                        "2. Buy the shield.\n"
                        "3. Check items.\n"
                        "4. Leave.\n")
    if market_choice == '1':
...
def Bandit_options():
    while True:
        bandit_response = input("1. Ask him to help rid the village of the monster.\n"
                                "2. Tell him he stinks and needs a bath.\n"
                                "3. Apologize and say you have the wrong house.\n")
        if bandit_response == '1':
            print_pause("The man responds saying they do not work for free. If\n"
                        "you have 100 gold they'd be more than happy to help.\n")
            pay_bandit = input("Do you accept?\n"
                            "1. Yes\n"
                            "2. No\n")
            if pay_bandit == '1':
                if items[0] >= 100:
                    items[0] = items[0] - 100
                    print_pause("The large man happily accepts and hands you a glowing \n"
                                "orb. He explains that if you throw it on the ground \n"
                ...
                print_pause("The large man spits on the ground and tells you to \n"
                            "take off.\n")
                break
            else:
                Not_a_choice()
                ...
                print_pause("The large mans tattoos begin to glow and he pushes you onto the ground\n"
                            "without even touching you.\n")
                print_pause("He says a lot of rude things and then slams the door shut.\n")
                items.append("bandits curse")
                Right_path_options()
                break
        elif bandit_response == '3':
...
def Right_path_options():
    while True:
        print_pause("What would you like to do?")
        bandit_choice = input("1. Knock on the front door.\n"
                            "2. Leave\n")
        if bandit_choice == '1':
            if 'bandits curse' in items:
                print_pause("A strange force is keeping you from approaching the house.\n")
                Right_path_options()
                break
            if 'bandit orb' in items:
                print_pause("The large man opens the door and is confused as to why you're still there.")
                Right_path_options()
                break
            else:
                print_pause("You knock on the front door.\n")
                print_pause("After a few moments a large hairy man with strange tattoos\n"
                            "opens the front door.\n")
                print_pause("He very rudely asks why you're on their property.\n")
                print_pause("What do you do?")
                Bandit_options()
                break
        if bandit_choice =='2':
...
def Cave_options():
    while True:
        ...
        if cave_choice == '1':
                ...
                print_pause("As you walk inside you notice horrible odors and bits of raw, uneaten food.\n")
                print_pause("Before too long a giant Hydra rises from the back of the cave!!\n")
                fight_hydra = input("What do you do??\n"
                                    "1. FIGHT! (you must roll a 15 or higher on a 20 sided dice)\n"
                                    "2. RUN AWAY!!\n")
                if fight_hydra == '1':
                    dice_roll = random.randint(1, 20)
                    if "steel sword" in items:
                        dice_roll = dice_roll + 5
                        print_pause("Your new sword shines brightly in the dark cave!")
                    if "steel shield" in items:
                        dice_roll = dice_roll + 5
                        print_pause("The steel shield holds strong against the enemy!")
                    if "bandit orb" in items:
                        dice_roll = dice_roll + 5
                        print_pause("You throw the bandit orb to the ground and summon your new allies!")
                    print(dice_roll)
                    if dice_roll >= 15:
                        print_pause("As the ferocious monster attacks and snaps, you manage to\n"
                                    "dodge and get close enough to stabe the beast in the heart!\n")
                        print_pause("It falls to the ground and exhales its last breath.")
                        print_pause("You are victorious!")
                        Play_again()
                        break
                    else:
                        print_pause("As valiant as you are, one of the beasts heads grabs you from\n"
                                    "behind and throws you into the air before swallowing you whole!")
                ...
                print_pause("You decide to leave for now and come back another time.\n")
                Adventure_choice()
        else:
            Not_a_choice()
...
def Tavern():
    ...
    print_pause("There's a lady working behind the bar.\n"
                "Also there's an old man rolling some dice at a table by himself.\n")
    Tavern_options() 
...
def Market():
    ...
    print_pause("He notices that your sword is rusty and your shield\n"
                "is made of wood.\n")
    print_pause("He pulls a shiny steel sword and shield from his cart.\n")
    print_pause("He offers them to you. 50 gold for the sword and 100 gold for the shield.\n")
    Market_options()

def Right_path():
    print_pause("You take the path leading to the right out of the village.\n")
    print_pause("Eventually you stumble across a small cabin with bits\n"
                "of trash and empty bottles scattered around the front.\n")
    Right_path_options()

def Left_path():
    print_pause("You take the path leading to the left out of the village.\n"
                "You notice large, strange footprints along your way.\n")
    print_pause("Eventually you find a cave with strange odors coming from it.")
    Cave_options()
